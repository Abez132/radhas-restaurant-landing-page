    ...
    <section id="about" className="relative overflow-hidden bg-cream py-24 md:py-32">
      <div className="mx-auto grid max-w-6xl gap-16 px-6 md:grid-cols-2 md:items-center" lang={lang}>
        <Reveal>
          ...
          <h2 className={`mt-4 font-display text-3xl leading-snug text-maroon-deep sm:text-4xl ${ethiopic}`}>
            {t.about.title}
          </h2>
          ...
          <div className="mt-8 flex gap-10">
            <div>
              <p className="font-display text-3xl text-maroon">{t.about.statRatingValue}</p>
              <p className={`mt-1 text-xs uppercase tracking-wider text-charcoal/60 ${ethiopic}`}>
                {t.about.statRatingLabel}
              </p>
            </div>
            <div>
              <p className="font-display text-3xl text-maroon">{t.about.statHoursValue}</p>
              <p className={`mt-1 text-xs uppercase tracking-wider text-charcoal/60 ${ethiopic}`}>
        ...
        <Reveal delay={2}>
          <div className="relative mx-auto flex aspect-[4/5] max-w-md items-center justify-center rounded-[2rem] border border-saffron/30 bg-gradient-to-br from-maroon to-maroon-deep p-10 shadow-xl">
            ...
            <span className="absolute -top-5 right-6 h-16 w-16 overflow-hidden rounded-full ring-4 ring-cream shadow-lg">
              <Image src="/images/logo.png" alt="Radha's logo" fill sizes="64px" className="object-cover" />
            </span>
            <div className="text-center">
              <p className={`font-display text-2xl italic text-saffron-soft ${ethiopic}`}>{t.about.quote}</p>
              <KalkaDivider className="mt-6" />
              <p className={`mt-6 font-body text-sm tracking-wide text-cream/80 ${ethiopic}`}>
                {t.about.quoteSource}
              </p>
            </div>
          </div>
